...
# query{
#   coupon(filter_by_status:1,filter_by_name: ""){
#     id
#     name
#     code
#     value
...
export default function Coupons() {
  ...
  const { data, error, refetch } = useQuery(GET_COUPONS,{
    variables:{filter_by_status:1,filter_by_name:null}
  });
  if (error) {
  ...
  function handleSelect({ value }) {
    ...
    setStatus(value);
    if (value.length) {
      refetch({ filter_by_status: value[0].value.toLowerCase() ==='active'?1:-1,filter_by_name:search
        // , searchBy: search
      
      
      });
    } else {
      refetch({ filter_by_status: 1 ,filter_by_name:search});
    }
  }
  ...
  function handleSearch(event) {
    ...
    setSearch(value);
    refetch({
      filter_by_status: status.length ? status[0].value : null,
      filter_by_name: value,
    });
  }
  function onAllCheck(event) {
    if (event.target.checked) {
      const idx = data && data.coupon.map(coupon => coupon.id);
      setCheckedId(idx);
    } else {
      setCheckedId([]);
    }
          ...
          <Wrapper style={{ boxShadow: '0 0 5px rgba(0, 0 , 0, 0.05)' }}>
            <TableWrapper>
              <StyledTable $gridTemplateColumns=' minmax(70px, 70px) minmax(200px, auto) minmax(200px, auto) minmax(200px, max-content) minmax(150px, auto) minmax(150px, auto) minmax(150px, auto)'>
                ...
                <StyledHeadCell>Coupon Name</StyledHeadCell>
                ...
                {data ? (
                 Object.keys(data.coupon).length>0 ? (
                    data.coupon
                          ...
                          <React.Fragment key={index}>
                            ...
                            <StyledBodyCell>
                              <ProgressWrapper>
                                ...
                                <ProgressText>{`${row[3] ? row[4]-row[3] : 0} of ${
                                  row[4]
